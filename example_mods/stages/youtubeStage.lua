...
function onCreatePost()
    ...
    makeText({tag = "commentsTotal", text = "100 Comments", cam = "camBYoutube", width = 200, size = 14, x = 55, y = 875, borderQuality = 0, sFactorX = 1, sFactorY = 1, antialiasing = false})
    ...
    precacheImage(folder .. "profiles/HaterMD"); precacheImage(folder .. "profiles/PlayerMD")
end
...
function sendMessage(isPlayer)
    ...
    if oldMustHit ~= isPlayer and #getTextString(curTxt) > 2 then 
        ...
        local tagPFP = "commentPFP" .. #comments 
        local tagTxt = "commentTxt"  .. #comments
        ...
        table.insert(comments, 1, {pfp = tagPFP, text = tagTxt, isPlayer = isPlayer == false})
...
function commentType(isPlayer ,id, dir, type, sus)
    ...
    if not dontSing then 
        ...
        local data = 1
        local isFound = false
        local vocalsN = isPlayer and vocalsNotes.bf or vocalsNotes.dad 
    ...
    if not dontSing then 
        ...
        if (type == "Hurt Note") and not isPlayer then 
            local singTexts = swears[nobadwordOption and 2 or 1][_G[numVocalName]][getRandomInt(1, #swears[nobadwordOption and 2 or 1][_G[numVocalName]])]
...
function opponentNoteHit(id, dir, type, sus)
    ...
    if type == "Hurt Note" then 
        ...
        if getHealth() > 0.2 and mechanicOption then 
